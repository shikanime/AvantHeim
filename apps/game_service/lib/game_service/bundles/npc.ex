defmodule GameService.NpcBundle do
  ...
  @type t :: %NpcBundle{
          ...
          # Optional components
          speed: maybe_component(E.SpeedComponent),
          quest: maybe_component(M.QuestComponent),
  ...
  defp npc_specs(%{vnum: vnum, dialog_id: dialog_id}) do
    [vnum: vnum, dialog_id: dialog_id]
  end

  defp position_specs(%{map_npc_id: map_npc_id, pos_x: map_x, pos_y: map_y} = attrs) do
    map_ref = Map.get(attrs, :map_ref, map_npc_id)
    [map_npc_id: map_npc_id, map_ref: map_ref, map_x: map_x, map_y: map_y]
  end

  defp direction_specs(attrs) do
    [value: Map.get(attrs, :direction_facing, :south)]
  end
end
